...
PDF는 최대 20페이지이며 Base64 JSON 업로드는 허용하지 않는다. 20MB는 20,000,000바이트다. 임시 작업은 생성 후 600초 동안 유효하다.
...
2026-09-08 배포 계약: itemCode는 실제 마스터 코드(예: FASTING_GLUCOSE)를 그대로 사용한다. 미매칭 코드는 null이며 검수 시 제외한다. measuredAt·providerName·confidence는 인식 결과에 따라 null일 수 있다. confidence를 임의로 계산하지 않는다. 확정 시 measuredAt는 필수이고 providerName은 선택이다. 완료 전 result는 null이다. pollAfterMs를 따르되 expiresAt 이후 조회를 중단한다.
...
<!-- 작성자: 김진우 -->
구현 상세·합성 응답·nullable·커서 오류 규칙은 [검진 회차 목록 API 구현 계약](검진_회차목록_API_구현계약.md)을 따른다. 공통 2.5 규칙에 따라 `data`는 배열이고 `meta`에 `nextCursor`, `hasNext`, `limit`을 반환한다. 검진일 없는 회차는 마지막에 정렬하며 소견 전용 회차도 포함한다.
...
- 공유 DB 마이그레이션은 미적용이다. 로컬 합성 검증을 원격 적용·배포·OCR/프런트 종단 간 완료로 해석하지 않는다.
