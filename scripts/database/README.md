...
> 후속 점검: SSM의 실행 컨테이너 설정 진단은 성공했고 DB·인증 프로젝트 일치 및 역할 postgres를 확인했다. Supabase Free 플랜에는 사용 가능한 백업/PITR 지점이 없다. CLI 2.117.0용 트랜잭션 경계 없는 생성 파일과 실행 조건은 `docs/requests/검진_버전2_접속대상_복구수단_확인결과.md`를 따른다. 기본 BEGIN/COMMIT 포함 파일을 CLI에 그대로 넘기면 이력 등록이 분리될 수 있다. 공유 DB 적용은 아직 실행하지 않았다.
